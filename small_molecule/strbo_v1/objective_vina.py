...
- :data:`Scorer` -- type alias for the canonical
  ...
  :mod:`strbo_v1.scorer` and re-exported here for backward compatibility

Everything else (``vina_dock_one`` / ``vina_dock_batch`` /
``_resolve_vina_bin`` / receptor helpers) is importable via
``strbo_v1.objective_vina.<name>`` but is intentionally not part of the
...
from strbo_v1.scorer import Scorer  # re-exported in __all__ below
...
class VinaScorerConfig:
    ...
    cache_dir: Path = Path("docking_work")
