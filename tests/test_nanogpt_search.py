...
from tasks.nanogpt.ldm_task import mock_train
...
from tasks.nanogpt.ldm_task.search_methods import beam_search, best_of_n, mcts, tree_search
from tasks.nanogpt.ldm_task.single_search import default_run_name, make_unique_run_dir, parse_args, safe_path_tag
...
class TestMockTrainAndSingleSearch:
    ...
    def test_single_search_name_and_argument_helpers(self, tmp_path: Path) -> None:
        ...
        with patch("tasks.nanogpt.ldm_task.single_search.time.strftime", return_value="STAMP"):
...
class TestOperationMockSearch:
    ...
    def test_generation_uses_a_semaphore_bound_to_the_active_loop(self, tmp_path: Path) -> None:
        task_root = Path(mock_train.__file__).resolve().parents[1]
        schema = load_operation_schema(
            task_root / "ldm_task" / "operation_schema_mock_train.json",
            task_root,
        )
