...
context becomes the prompt and the accepted proposal, together with its
...
The example config here fine-tunes `Qwen/Qwen3.5-9B`, but the recipe is
model-agnostic: point `model_name_or_path` at any LLaMA-Factory-supported model
and adjust `template` accordingly. See
[`../DATA_COLLECTION.md`](../DATA_COLLECTION.md) for how the search trajectories
are collected and rendered into the Alpaca dataset.
...
config defaults to `flash_attn: sdpa`, which needs no extra dependency and runs
anywhere.

## 2. Prepare the dataset

Render the collected search trajectories into an Alpaca-format SFT file following
[`../DATA_COLLECTION.md`](../DATA_COLLECTION.md), and place it in
`finetune/data/`:
...
`dataset_info.json` already registers this file as the `ldm_mixed_cot` dataset.
Each row is a standard Alpaca record:

```json
{"instruction": "<round context / task prompt>", "input": "", "output": "<accepted proposal + rationale>", "system": "<system prompt>"}
```
...
Checkpoints are written to `output/ldm_cot/`. To log to Weights & Biases, set
`report_to: wandb` and add a `run_name` in the config.
