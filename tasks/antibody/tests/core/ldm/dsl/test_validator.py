"""tests/core/ldm/dsl/test_validator.py"""
from __future__ import annotations

from tasks.antibody.core.ldm.dsl.search_space import LocalSearch, NeighborSampling, Or
...
class TestValidateBias:
    def test_ok(self):
        from tasks.antibody.core.ldm.dsl.bias import MaxCysteine
        assert validate_bias_atom(MaxCysteine(1)) == []

    def test_too_deep(self):
        from tasks.antibody.core.ldm.dsl.bias import BiasSum, MaxCysteine
        atom = MaxCysteine(1)
        for _ in range(5):
