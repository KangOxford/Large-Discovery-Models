"""LDMEngine campaign assembly for adaptive KV-cache quantization."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_dir = (
        ...
        else unique_run_dir(args.out_dir / (args.run_name or _default_run_name(args, workloads)))
    )
    ...
    if args.proposal_mode == "openai":
        if not args.llm_url or not args.llm_model_name:
            ...
            payload["status"] = "paused_endpoint_unavailable"
            print(json.dumps(payload, indent=2, sort_keys=True))
            return 2
    ...
    if args.mock:
        ...
        if upstream_root is None or package_dir is None:
            runtime.fail("Real evaluation requires --upstream-root and --package-dir.")
