"""Canonical scoring interface shared by all ``tasks.small_molecule.core`` backends.
...
The concrete implementations live in ``tasks.small_molecule.core.objective_vina``
(:class:`VinaScorer`, AutoDock Vina with disk cache) and
``tasks.small_molecule.core.objective_nn`` (:class:`NNScorer`, the G12C pIC50 ensemble
from ``resources/models/best_g12d_model.joblib``). Both re-export
