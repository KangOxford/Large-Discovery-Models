...
"""
python tasks/nanogpt/run_expanded_search.py \
    --train-file tasks/nanogpt/ldm_task/real_train.py \
    --generator operation_tool \
    --operation-schema tasks/nanogpt/ldm_task/operation_schema_real_train.json \
    --method best_of_n \
    --breadth 4 \
    --depth 4 \
    --iterations 100 \
    --warmup 20  \
    --seed-policy best  \
    --buffer tasks/nanogpt/ldm_runs/ablation_buffer/expanded_exp1/gp_warmup.jsonl  \
    --out-dir tasks/nanogpt/ldm_runs/ablation_runs  \
...
from tasks.nanogpt.ldm_task.single_search import make_unique_run_dir, safe_path_tag
from tasks.nanogpt.ldm_task.search_core import (
...
class OperationSearchEngine(SearchEngine):
    ...
    async def _call_operation_generator(
        ...
        from tasks.nanogpt.ldm_task.api_generate import (
            add_usage,
...
async def async_main() -> int:
    ...
    if args.generator in OPERATION_GENERATORS and operation_schema is None:
        raise SystemExit(
            ...
            "or a default ldm_task/operation_schema_real_train.json / "
...
class GPSurrogate:
    ...
    def predict(self, vector: list[float], *, mode: str, beta: float, xi: float) -> Prediction:
        ...
        acquisition = make_acquisition(mode, minimize=(self.minimize,), beta=beta, xi=xi)
