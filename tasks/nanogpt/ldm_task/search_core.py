...
class SearchConfig:
    ...
    generator: str = "api"
    llm_url: str = "http://10.253.9.4:52306/v1"
...
class SearchEngine:
    ...
    async def _call_generator(self, prompt: str, state: SearchState, current_train_text: str) -> tuple[str, Any]:
        ...
        if self.config.generator == "closed_loop":
            from tasks.nanogpt.ldm_task.api_generate import openai_compatible_generate
        ...
        if self.config.generator == "tool_call":
            from tasks.nanogpt.ldm_task.api_generate import tool_call_generate
        ...
        if self.config.generator == "harness":
            from tasks.nanogpt.ldm_task.api_generate import harness_generate
        ...
        from tasks.nanogpt.ldm_task.api_generate import vllm_generate
