...
def validate_policy_json(path: str) -> dict[str, Any]:
    with open(os.path.abspath(path), "r", encoding="utf-8") as handle:
        policy = json.load(handle)
    if not isinstance(policy, dict):
        raise ValueError(f"Policy at {path} must be a JSON object")
    allowed = {"trust_region_centers", "mutation_policy", "soft_constraints", "antigen_preferences"}
    unknown = set(policy) - allowed
    if unknown:
        raise ValueError(f"Policy at {path} contains unknown top-level keys: {sorted(unknown)}")
    return policy


def run_llm_policy_command(
    command_template: str,
    prompt_path: str,
    output_path: str,
    timeout_s: int = 120,
) -> dict[str, Any]:
    """Run a provider-agnostic external command that writes a policy JSON.

    The template may contain `{prompt_path}` and `{output_path}` placeholders. Example:
        python scripts/call_llm.py --input {prompt_path} --output {output_path}
    """
    os.makedirs(os.path.dirname(os.path.abspath(output_path)), exist_ok=True)
    command = command_template.format(
        prompt_path=shlex.quote(os.path.abspath(prompt_path)),
        output_path=shlex.quote(os.path.abspath(output_path)),
    )
    proc = subprocess.run(
        command,
        shell=True,
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"LLM policy command failed: {result}")
    validate_policy_json(output_path)
    return result
