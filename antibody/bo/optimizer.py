...
class Optimizer:
    ...
    def __init__(self,
        ...
        self.initial_points = None if initial_points is None else np.asarray(initial_points, dtype=int)
        if self.initial_points is not None:
            if self.initial_points.ndim != 2:
                raise ValueError(f"initial_points must be 2D, got shape {self.initial_points.shape}")
            if self.initial_points.shape[1] != len(config):
                raise ValueError(
                    f"initial_points seq_len {self.initial_points.shape[1]} does not match config length {len(config)}"
                )
            if np.any(self.initial_points < 0) or np.any(self.initial_points >= config.reshape(1, -1)):
                raise ValueError("initial_points contain amino-acid indices outside configured category ranges")    
    ...
    # Pickling support: drop the LDM orchestrator from kwargs. The orchestrator
    # holds an openai.OpenAI client (with a non-picklable threading.RLock).
    # It is a runtime resource that must be reconstructed after unpickle;
    # the caller (BOExperiments.load) is responsible for re-injecting it.
    _PICKLE_SKIP_KEYS = ("orchestrator",)
    ...
    def __getstate__(self):
        ...
        if isinstance(kw, dict):
            state["kwargs"] = {
                k: v for k, v in kw.items() if k not in self._PICKLE_SKIP_KEYS
            }
        return state
    ...
    def restart(self) -> None:
        ...
        if self.initial_points is not None and self.casmopolitan.n_evals == 0:
            if len(self.initial_points) < self.casmopolitan.n_init:
                raise ValueError(
                    f"Need at least {self.casmopolitan.n_init} initial_points, got {len(self.initial_points)}"
                )
            self.casmopolitan.restart()
            self.casmopolitan.tr_x = np.zeros((0, self.casmopolitan.dim))
            self.casmopolitan.tr_fx = np.zeros((0, 1))
            self.x_init = deepcopy(self.initial_points[:self.casmopolitan.n_init])
            if self.table_of_candidates is not None:
                self.table_of_candidates, self.table_of_candidate_embeddings = update_table_of_candidates(
                    ...
                    observed_candidates=self.x_init,
        ...
        if self.guided_restart and len(self.casmopolitan.tr_fx) and self.kwargs['search_strategy'] == 'local':
            ...
            if self.table_of_candidates is not None:
                ...
                while itern < self.casmopolitan.n_init:
                    ...
                    itern += 1
                self.x_init = np.vstack([
            (x.detach().cpu().numpy() if hasattr(x, "detach") else np.asarray(x)).reshape(-1)
