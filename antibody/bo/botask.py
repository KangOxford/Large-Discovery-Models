...
from task.tools import Absolut, Manual, TableFilling, RandomBlackBox, SmokeAntibody
...
class BOTask(TestFunction):
    ...
    def __init__(self,
        ...
        if self.bbox.get('antigen', '').upper().startswith('SMOKE'):
            self.bbox['seq_len'] = self.dim
            self.fbox = SmokeAntibody(self.bbox)
        elif self.bbox['tool'] == 'Absolut':
