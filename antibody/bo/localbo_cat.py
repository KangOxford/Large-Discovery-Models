...
class CASMOPOLITANCat:
    # Pickling support: exclude the orchestrator from pickling because
    # its LLMClient holds non-picklable internal state (e.g. openai's
    # RLock). The orchestrator is re-injected by the caller after unpickling.
    def __getstate__(self):
        state = self.__dict__.copy()
        state["orchestrator"] = None
        state["_last_session"] = None
        kwargs = state.get("kwargs")
        if isinstance(kwargs, dict) and "orchestrator" in kwargs:
    ...
    def __init__(
        ...
        self.antigen = None
        # LDM: optional Orchestrator injection (lazy import to avoid polluting
        # the original AntBO code path when bo.ldm is not used).
        self.orchestrator = kwargs.get('orchestrator')
        self.antigen_id: str = kwargs.get('antigen_id', 'unknown')
        self.antigen_seed: int = int(kwargs.get('antigen_seed', 0))
        self.iter_seed: int = int(kwargs.get('iter_seed', 0))
        self.antigen_context: dict = kwargs.get('antigen_context', {})
        # Current DSL state (updated by orchestrator each iteration)
        self._search_dsl = None  # SearchSpaceAtom | None
        self._bias_dsl = None    # BiasAtom | None
        self._iteration_count = 0
        self._tr_multiplier_count = 0
    ...
    def create_and_select_candidates(
        ...
        # --- LDM Orchestrator integration (lazy import) ---
        if self.orchestrator is not None:
            from bo.ldm import build_status, apply_decision
            self._iteration_count += 1
            status = build_status(
                self,
                antigen_id=self.antigen_id,
                antigen_seed=self.antigen_seed,
                iter_seed=self.iter_seed + self._iteration_count,
                antigen_context=self.antigen_context,
            )
            decision = self.orchestrator.step(status)
            apply_decision(self, decision)
            self._last_decision = decision

            # Adaptive fallback DSL: if LLM didn't provide update_trust_region,
            # generate a default LocalSearch around the best observed point.
            # Sized to fit within acq_search_budget.
            if self._search_dsl is None:
                from bo.ldm.dsl.search_space import LocalSearch
                budget = int(self.kwargs.get('acq_search_budget', 600))
                restart = 3
                steps = max(1, budget // restart - 1)
                best_idx = int(fx.argmin())
                best_x = x[best_idx]
                center_seq = "".join(
                    "ACDEFGHIKLMNPQRSTVWY"[int(c)] for c in best_x
                )
                self._search_dsl = LocalSearch(
                    center_seq, radius=3, restart=restart, steps=steps
                )

        # Path is config-driven: llm_loop_enabled determines which path runs.
        # _search_dsl is guaranteed to be set when llm_loop_enabled=True
        # (either by LLM or by the adaptive fallback above).
        _ldm_active = (
            self.kwargs.get('llm_loop_enabled', True)
            and table_of_candidate_embeddings is None
        )
        ...
        # ================================================================
        # Path B: LDM interactive acquisition session
        # ================================================================
        if _ldm_active and self.acq in ['ei', 'ucb'] and table_of_candidate_embeddings is None:
            from bo.ldm.acquisition.session import AcquisitionSession
            from bo.ldm.config import DSLConfig

            f_acq = _ei if self.acq == 'ei' else _ucb
            _llm_cfg = self.kwargs
            _dsl_cfg = DSLConfig(
                acq_search_budget=int(_llm_cfg.get('acq_search_budget', 600)),
                acq_max_rounds=int(_llm_cfg.get('acq_max_rounds', 3)),
                num_llm_review=int(_llm_cfg.get('num_llm_review', 10)),
                max_retries=int(_llm_cfg.get('max_retries', 3)),
                bias_weight=float(_llm_cfg.get('bias_weight', 0.1)),
                sample_timeout_s=float(_llm_cfg.get('sample_timeout_s', 5.0)),
                batch_size=self.batch_size,
                strategy=str(_llm_cfg.get('strategy', 'ldm-default')),
                atoms_whitelist=tuple(_llm_cfg.get('atoms_whitelist', (
            ...
            _status_data = {
                ...
                "best_value": float(fx.min()) if len(fx) > 0 else 0.0,
                "strategy": _dsl_cfg.strategy,
            }

            x_next = session.run(
                search_dsl=self._search_dsl,
                bias_dsl=self._bias_dsl,
                gp=gp,
                f_acq=f_acq,
                batch_size=self.batch_size,
                cat_config=self.config,
                cdr_constraints=self.cdr_constraints,
                device=device,
                status_data=_status_data,
            )
            acq_next = np.array([0.0] * len(x_next))
            self._last_session = session

        # ================================================================
        # Path A: Original AntBO acquisition (verbatim from upstream)
        # ================================================================
        if not _ldm_active:
