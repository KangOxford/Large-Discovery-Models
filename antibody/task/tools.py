...
class Absolut(BaseTool):
    def __init__(self, config) -> None:
        ...
        # Unique prefix per Python process for concurrent-run isolation.
        # Combined with ms timestamp in energy() to form:
        #   run{pid}_{epoch_ms}_<rest>
        # The prefix arg is supported natively by Absolut's `repertoire` command
        # (see src/Absolut/delimain.cpp option2, argc==6 branch): it is prepended
        # to every TempCDR3 / TempBindingsFor / FinalBindings filename, so two
        # BO runs sharing the same Absolut install dir never collide.
        self._pid = os.getpid()
    ...
    def energy(self, x: np.ndarray) -> tuple[np.ndarray, list[str]]:
        ...
        antigen = self.config['antigen']
        process = self.config['process']
        # pid + ms timestamp: unique across concurrent runs AND across repeated
        # calls within the same Python process (subprocess.run is synchronous,
        # so two calls cannot share a millisecond).
        prefix = f"run{self._pid}_{int(time.time() * 1000)}_"

        input_file  = f"{prefix}TempCDR3_{antigen}.txt"
        temp_files  = [f"{prefix}TempBindingsFor{antigen}_t{i}_Part1_of_1.txt"
                       for i in range(process)]
        # Absolut is invoked without mpirun, so MPI nJobs==1 and exactly one
        # output file is produced regardless of the thread count `process`.
        output_file = f"{prefix}{antigen}FinalBindings_Process_1_Of_1.txt"
        all_files = [input_file] + temp_files + [output_file]

        current_dir = os.getcwd()
        os.chdir(self.config['path'])

        # NOTE: no pre-cleanup. Stale files from crashed runs are left in place
        # and collected by scripts/clean_absolut_temp.sh (matches prefix +
        # exact suffix patterns to avoid touching Absolut's data files).
        ...
        result = subprocess.run(
            ['taskset', '-c', f"{self.config['startTask']}-{self.config['startTask'] + process}",
             "./src/bin/Absolut", 'repertoire', antigen, input_file,
             str(process), prefix], capture_output=True, text=False)
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"Absolut subprocess failed (returncode={result.returncode}):\n{stderr}"
            )
        ...
        if not os.path.exists(output_file):
            ...
            raise FileNotFoundError(
                f"Absolut completed but output file not found: {output_file}"
            )

        data = pd.read_csv(os.path.join(self.config['path'], output_file),
                           sep='\t', skiprows=1)
        ...
        energy = data.groupby(by=['sequence_idx'])[['Energy']].min()
        min_energy = energy['Energy'].values

        # Post-call cleanup: remove only this call's files (prefix-scoped).
        for f in all_files:
            if os.path.exists(f):
                os.remove(f)
        os.chdir(current_dir)
...
class SmokeAntibody(BaseTool):
    """Synthetic multi-peak landscape for fast BO validation.

    Activated when antigen starts with 'SMOKE'.  Uses Hamming-distance-
    decayed peaks so the landscape has realistic multi-modal structure:

        f(x) = Σ_p  amplitude_p × exp(−hamming(x, center_p) / scale_p)

    Peaks are seeded for reproducibility.  Energy range ≈ −30 to −100,
    matching typical Absolut binding energies.
    """

    def __init__(self, config) -> None:
        BaseTool.__init__(self)
        self.config = config
        self.antigen = config['antigen']
        seq_len = config.get('seq_len', 11)
        n_peaks = config.get('n_peaks', 5)
        seed = config.get('seed', 42)

        rng = np.random.default_rng(seed)
        self.centers = rng.integers(0, 20, size=(n_peaks, seq_len))
        self.amplitudes = rng.uniform(-80.0, -50.0, size=n_peaks)
        self.scales = rng.uniform(1.5, 3.0, size=n_peaks)
    ...
    def energy(self, x: np.ndarray) -> tuple[np.ndarray, list[str]]:
        ...
        for i, seq in enumerate(x):
            sequences.append(''.join(self.idx_to_AA[int(a)] for a in seq))
            total = 0.0
            for p in range(len(self.amplitudes)):
                hamming = int(np.sum(seq != self.centers[p]))
                total += self.amplitudes[p] * np.exp(-hamming / self.scales[p])
            results[i] = total

        return results.reshape(-1, 1), sequences
