...
def fig1(data):
    ...
    cap=('读图：上方箭头只表示流程顺序，四个框分别对应生成、真实评估、优化记录和保存；它们不是四组对照实验。'
      ...
      '梯度范数衡量反向传播信号的大小；虚线 1 是裁剪阈值，不是学习有效性的判定线；纵轴为对数刻度。1.5B 指约 15 亿参数，RL 指强化学习。\n'
      '范围：单次小模型链路验证，K=2（每个任务两条候选轨迹），30 轮采样、60 次优化记录。奖励是 acquisition（候选的预期改进评分），'
      '不是最终分子搜索成绩；real 模式使用 Vina 分子对接和 KRAS G12D 突变体的活性预测。有限梯度和 checkpoint（模型存档）只能支持链路验证，不能证明 RL 提升。\n'
    ...
    ax=fig.add_axes([.045,.62,.91,.23]);ax.set_axis_off();ax.set_xlim(-.01,1.01)
...
def fig2(data):
    cap=('读图：左侧折线展示各运行的裁剪前梯度范数；纵轴每升一格代表放大 100 倍，横轴是各自的优化尝试序号，不是相同训练预算。'
      ...
      '右侧蓝段表示有限梯度记录，红段表示 NaN/Inf（非数值／无穷大）；长度是次数，数字标明分子／分母。梯度范数衡量反向传播信号大小。\n'
...
def fig3(data):
    cap=('读图：左侧每根柱的分母均为 100 个任务组（50 轮采样 × 每轮 2 组），柱高表示组内所有候选奖励都为零的比例。'
      ...
      '定义：9B 为约 90 亿参数，SFT 为监督微调，acquisition 是预期改进评分，token 是文本编码单位，NaN 为非数值。'
      '共同条件：9B SFT 起点、真实 acquisition 奖励、回复上限 8192 token、50 轮采样。K=2/U=1 与 K=8/U=2 同时改变了每任务候选数 K 和每轮更新次数 U；'
...
def fig4(data):
    cap=('读图：箭头表示形成效果结论所需的证据顺序，不表示训练耗时或已经完成的比例。蓝色框表示已观察到的工程产物，橙色框表示尚未完成的训练验证，'
      ...
      '定义：1.5B/9B 为约 15 亿/90 亿参数，checkpoint 是模型存档，seeds 是随机种子。SFT-only 为仅监督微调的模型，SFT+RL 为其上继续强化学习的模型；训练目标为 KRAS G12D。'
