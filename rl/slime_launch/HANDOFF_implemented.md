...
- **reward 未通过** → [01_reward_always_zero.ipynb](handoff_notebooks/01_reward_always_zero.ipynb)
  ...
  **格式修复已验证**：把观测包成独立的 user turn 之后，
  **解析失败率 81.1% → 0.0%**（1249 次失败变成 0 次），
  **零方差组 0.69 → 0.39 每步**（降 43%）。判定标准是在看到结果之前
  写下的，其中「零方差只降不清零就要转向查 `n_samples_per_prompt`
  与候选去重」这一条现在生效了——**格式不是零方差的唯一成因**。
