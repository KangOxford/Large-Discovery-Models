...
成因有两个、而且叠加：**组里只有两条轨迹**（定基线高度），
以及 **GP 历史变长**（定它随训练爬多快）。把 `n_samples_per_prompt`
从 2 提到 8，零方差组从每步 0.94 降到 0.02，两个成因同时被压住
（四档实测 + 同 run 前后半对照，见 §6 P0）。**P2 起跑前把这个值设成 8 即可**，
不需要 16。
...
- **reward 这样没过** → [01](handoff_notebooks/01_reward_always_zero.ipynb)
  ...
  **那个测量做完了，成因定了**——四档除组内轨迹数外一切相同，同期起跑：
  ...
  | n_samples | run 数 | 步数 | 零方差组/步 | 绝对次数 |
  |---:|---:|---:|---:|---:|
  | 2 | 3 | 63 | **0.94** | 59 |
  | 4 | 2 | 32 | **0.28** | 9 |
  | 8 | 8 | 51 | **0.02** | 1 |
  ...
  **0.94 → 0.28 → 0.02 → 0.09，n≥8 基本消除。** 道理很直白：
  GRPO 的梯度来自组内差异，一组只有两条轨迹时，两条撞出同一个分子
  就足以让标准差为 0。

  n=16 比 n=8 高一点，读起来像非单调。但绝对次数是 **1 次对 2 次**，
  落在噪声里，不构成「16 比 8 差」。**稀有事件用计数说，别用率说**——
  1/51 和 2/23 写成率差 4 倍，写成次数就只差 1 次。

  **对 P2**：`n_samples` 直接乘在每步的 rollout 成本上，4 压掉 70%，
  8 之后基本消除，**16 没有额外收益、只是更贵**。

  **还有第二个成因：GP 历史变长。** 读数随训练推进上升原本只是趋势，
  单条曲线上「随训练上升」与「这一档本来就高」分不开。用**同一个 run 的
  前后半**比就分开了（run 内 n 固定，差异只能来自训练进度）：

  | run | n | 步数 | 前半 | 后半 |
  |---|---:|---:|---:|---:|
  | zs-n2-3 | 2 | 60 | 0.80 | **1.13** |
  | zs-n4-3 | 4 | 22 | 0.00 | **0.55** |
  | zs-n16-5 | 16 | 13 | 0.00 | **0.29** |
  | zs-n4-4 | 4 | 10 | 0.20 | **0.40** |
  | zs-n8-7 | 8 | 22 | 0.00 | **0.09** |
  | zs-n8-8 | 8 | 13 | 0.00 | 0.00 |

  **六个 run 升五降零平一**，加权 0.36 → 0.64。与 EHVI 随 GP 历史衰减吻合。

  **两个成因叠加**：`n_samples` 定基线高度，GP 历史长度定它爬多快。
  把 n 提到 8 同时压住两者。

  （数据截至 2026-09-03 08:2x UTC，两个 run 仍在跑，数字还会小幅变动。
  结论不受影响。）
